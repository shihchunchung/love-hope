...
`index_v3.html` — 首頁、所有共用元件的 source of truth：
- `.article-card` — 列表卡片
- `.trend-spread` — 1+4 不對稱排行
- `.action-row` — 平等的水平條紋
- `.spot-card` — 1:1 兩欄重點卡
- `.donate-card` — contained 紅色 CTA
- `.section-head` / Header / Nav / Footer — 全站共用

`venue.html` — 場地租借子頁、子頁元件範本：
- `.page-hero` — 子頁靜態 hero
- `.venue-row` / `.venue-detail` — action-row / spot-card 的子頁變體
- `.pdf-block` — PDF 下載灰底卡
- `.closing-quote` — 灰底引言收尾

`index.html` / `index_v2.html` — 歷史版本，保留勿改。

精確 tokens、HTML 結構、CSS 細節：直接讀 v3／venue.html 的 `<style>` 與 `<body>`。

## 設計核心原則

1. **品牌色 #8C0A0A 是主視覺** — 學 La Vie 編輯語彙（hairlines / 4px / flat shadow / 60×3 underline / dark footer），不抄它的純黑白。
2. **不要連續堆疊重色塊** — 紅色強區段與黑色 footer 之間至少留 72px 白色緩衝（v2 教訓）。
3. **編輯感靠留白與克制** — 不靠 box-shadow 模糊、不靠漸層、不靠 emoji。
4. **View More 永遠在區段底**，置中圓角小按鈕。
5. **不自創文案** — 重新設計時用既有素材，不發明新詞。

## Hover 兩級系統（決策規則）

判斷：「這個動作是不是頁面上的『重點轉換』？」

- **是 → 高調**（整卡位移 + 平面 shadow + 數字 hollow→fill）：trend-feature / spot-card / venue-detail / donate-fact
- **否 → 低調**（圖 scale + 標題變紅）：article-card / trend-item / action-row / venue-row

## 子頁建立 SOP

1. Hero 用 `.page-hero`（單張靜態），不要用首頁的 peek 輪播
2. 連結三處：v3 nav / 支持卡片 / footer-links
3. 子頁 nav 對應項加 `class="active"`
4. 子頁 logo 連回 `index_v3.html`
5. 內容範圍：只用既有頁面有的內容；PDF 內容不硬搬
...
- ❌ 全站每個區段都加 icon
- ❌ 紅色用在文字本身
- ❌ c-tag 加底色／padding 變成 chip
- ❌ 重新設計子頁時，從 PDF 搬入既有頁面沒有的內容
- ❌ 自創新標題／新副標／新 CTA 文案
- ❌ 子頁 Hero 用首頁的 peek 輪播
